...
describe('Light Deemer Extension E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const extensionTarget = targets.find(
      (target: any) => target.type() === 'service_worker' && target.url().includes('chrome-extension://')
    );
    
    if (extensionTarget) {
      const extensionUrl = extensionTarget.url();
      extensionId = extensionUrl.split('chrome-extension://')[1]!.split('/')[0]!;
  ...
  test('should load extension and create overlay on page', async () => {
    ...
    await page.goto('data:text/html,<html><body style="background:white;height:100vh;"><h1>Test Page</h1></body></html>');
  ...
  test('should raise video elements when excludeVideos is enabled', async () => {
    ...
    await page.goto('data:text/html,<html><body><video width="320" height="240" controls><source src="movie.mp4" type="video/mp4"></video></body></html>');
  ...
  test('should hide overlay in fullscreen mode', async () => {
    await page.goto('data:text/html,<html><body style="background:white;height:100vh;"><button id="fullscreen-btn">Fullscreen</button></body></html>');
  ...
  test('should respect whitelist settings', async () => {
    ...
    await page.goto('data:text/html,<html><body style="background:white;height:100vh;"><h1>Whitelisted Page</h1></body></html>');
  ...
  test('should open options page and manage whitelist', async () => {
    ...
    const whitelistItem = await optionsPage.$('.whitelist-item .whitelist-domain');
    const whitelistText = await optionsPage.evaluate(el => el?.textContent, whitelistItem);
